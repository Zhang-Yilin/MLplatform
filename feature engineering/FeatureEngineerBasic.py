...
class PCA_select(Feature):
    ...
    def select_by_percentage(self, percentage):
        """
        按向量解释方差的百分比选择
    ...
    def _select_by(self, **type_arg):
        ...
        if type_arg["type"] == 0:
            ...
            self.select_column(*type_arg["columns"])
            return self.select_by_percentage(type_arg["typearg"])
        else:
...
class cut_select(Feature):
    ...
    def _cut_quantile(self, *quantile):
        """
        按设定分位数分箱
    ...
    def _select_by(self, **type_arg):
        ...
        :return: 所有输入列和分箱结果向量组成的数据框（包含输入表的所有数据） pandas.dataFrame
        """
        if len(type_arg["columns"]) != len(type_arg["typearg"]):
            raise ValueError("列数与分箱参数不匹配")
        if "suffix" in type_arg.keys():
            self.set_suffix(type_arg["suffix"])
        if type_arg["type"] == 0:
            self.select_column(*type_arg["columns"])
            return self._cut_number(type_arg["typearg"])
        elif type_arg["type"] == 1:
            self.select_column(*type_arg["columns"])
            return self._cut_width(type_arg["typearg"])
        elif type_arg["type"] == 2:
            self.select_column(*type_arg["columns"])
            return self._cut_quantile(type_arg["typearg"])
        elif type_arg["type"] == 3:
            self.select_column(*type_arg["columns"])
            return self._cut_sd(type_arg["typearg"])
        else:
            raise ValueError("type error：不存在所选类")
